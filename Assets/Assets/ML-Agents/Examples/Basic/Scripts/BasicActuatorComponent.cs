...
namespace Unity.MLAgentsExamples
{
    ...
    public class BasicActuator : IActuator
    {
        ...
        public void OnActionReceived(ActionBuffers actionBuffers)//NOTE Action 정의
        {
            var movement = actionBuffers.DiscreteActions[0];

            var direction = 0;

            //NOTE 1일떄 -1, 2일때 1, 0일떄 noop
            switch (movement)
            {
                case 1:
                    direction = -1;
            ...
            basicController.MoveDirection(direction); //NOTE 실제로 움직이는 라인
        }
        ...
        public void Heuristic(in ActionBuffers actionBuffersOut)
        {
            ...
            if (Mathf.Approximately(direction, 0.0f))//NOTE direction이 0에 가까우면 0으로 만듬
            {
                discreteActions[0] = 0;
                return;
            }
            var sign = Math.Sign(direction);//부호 리턴
